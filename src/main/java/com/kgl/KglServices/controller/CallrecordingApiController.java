...
public class CallrecordingApiController {
	...
	public Boolean ExotelCampaignAPI() throws Exception {
		...
		if (startTime.before(endTime)) {
			String AP_URL = AP_googlesheet_data_restapi;
			List<CampaignPojo> list = getCampData(AP_URL);
			int recordsSize = list.size();
			logger.info("AP IVR google sheet api records count:::" + recordsSize);
			if (recordsSize > 0) {
	...
	@GetMapping("/ApGoogleSheetUpdate")
